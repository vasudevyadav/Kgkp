...
const BlogDetailsContent = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4">
      <h1 className="text-3xl md:text-4xl font-semibold text-gray-900 mb-4">{blog.title}</h1>
      ...
      <img src={blog.image} alt={blog.title} className="w-full h-[380px] object-cover mb-6 rounded-lg" />
      ...
      {blog.tableOfContents?.length > 0 && (
        <div className="bg-gray-100 rounded-xl w-fit p-6 mb-10">
          <h3 className="text-lg font-medium text-gray-800 mb-4">Table of Contents</h3>
      ...
      {blog.sections?.map((section, index) => (
        <div key={index} id={`section-${index}`} className="mb-10">
          <h2 className="text-2xl font-medium text-gray-900 mb-3">{section.title}</h2>
      ...
      {blog.conclusion && (
        <div className="bg-gray-100 p-6 rounded-xl mt-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-2">Conclusion</h2>
      ...
      <div className="flex items-center gap-4 mt-10">
        {authorImage ? (
          <img src={authorImage} className="w-12 h-12 rounded-full object-cover" />
        ) : (
          <div className="w-12 h-12 bg-gray-300 rounded-full" />
        )}
        <div>
          <p className="text-base font-semibold text-gray-900">Author: {blog.author}</p>
