...
const ProjectGallery = ({ data }) => {
  ...
  const images = galleryData?.images?.map((img) => ({
    fullUrl: img.thumbImage || img.thumbUrl,
    thumbUrl: img.thumbImage || img.thumbUrl,
  })) || [];

  const heading = galleryData?.title || "Project Gallery";
  const tagline = galleryData?.tagline || "Gallery";
  const description =
